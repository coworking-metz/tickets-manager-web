<template>
  <AppDialogConfirm
    ...
    <AppTextareaField
      ...
      :label="$t('memberships.delete.comment.label')"
      :placeholder="$t('memberships.delete.comment.placeholder')"
      required />
...
const vuelidate = useVuelidate(rules, state, { $scope: 'memberships-delete' });
...
const onDelete = async () => {
  ...
  deleteMemberMembership(props.memberId, props.membershipId, state.comment as string)
    .then(() => {
      notificationsStore.addSuccessNotification(
        i18n.t('memberships.delete.onDelete.success', {
    ...
    .catch((error) => {
      notificationsStore.addErrorNotification(error, i18n.t('memberships.delete.onDelete.fail'));
