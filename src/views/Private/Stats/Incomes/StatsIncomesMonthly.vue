<template>
  <div class="mx-auto flex w-full max-w-5xl grow flex-col">
    <Head>
      <title>{{ $t('stats.incomes.monthly.head.title') }}</title>
    </Head>
    <section class="flex min-h-[320px] grow flex-col">
      ...
      <VueECharts v-else class="h-full w-full" :option="options" @click="onBarSelect" />
    </section>
    ...
    <section class="mx-6 lg:mx-8">
      ...
      <dl
        ...
        <div class="px-4 py-5 sm:p-6">
          ...
          <div
            ...
            <span class="shrink grow basis-0 truncate font-normal text-gray-500">
              {{
                $t('stats.incomes.monthly.summary.average.threshold', {
        ...
        <div class="px-4 py-5 sm:p-6">
          <dt class="truncate font-medium text-gray-500 sm:text-sm">
            {{ $t('stats.incomes.monthly.summary.total.label') }}
          </dt>
          <dd class="mt-1 flex flex-col">
            ...
            <div
              ...
              <span class="shrink grow basis-0 truncate font-normal text-gray-500">
                {{
                  $t('stats.incomes.monthly.summary.total.threshold', {
...
const MINIMUM_INCOME = MONTHLY_MINIMUM_INCOME;
        ...
        <dl class="flex flex-col gap-1">
          ...
          <div class="flex flex-row justify-between place-items-end">
            <dt class="flex flex-row gap-1 items-center text-base font-normal">
              ...
              ${i18n.t('stats.incomes.monthly.graph.tooltip.tickets.label', {
                count: usedTickets,
              })}
            </dt>
          ...
          <div class="flex flex-row justify-between place-items-end">
            <dt class="flex flex-row gap-1 items-center text-base font-normal">
              ...
              ${i18n.t('stats.incomes.monthly.graph.tooltip.subscriptions.label', {
                count: daysAbo,
              })}
            </dt>
...
const onBarSelect = ({ dataIndex }: { dataIndex: number }) => {
  ...
  router.push({
    ...
    query: {
      from: dayjs(date).startOf('month').format(DATE_FORMAT),
      to: dayjs(date).endOf('month').format(DATE_FORMAT),
    },
  });
};
