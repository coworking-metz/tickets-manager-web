<template>
  <div class="mx-auto flex w-full max-w-5xl grow flex-col">
    <Head>
      <title>{{ $t('stats.activity.monthly.head.title') }}</title>
    </Head>
                ...
                (count: number) =>
                  `${
                    count
                      ...
                      : ''
                  }${$t('stats.activity.monthly.summary.average.people.format', {
                    count,
                  })}`
              "
              ...
              :format="
                (count: number) =>
                  `${formatAmount(count, {
                    ...
                    style: 'decimal',
                  })}${$t('stats.activity.monthly.summary.average.attendance.format', {
                    count,
                  })}`
              "
...
import { ActivityPeriod, getActivityPerMonth } from '@/services/api/activity';
...
const state = reactive({
  ...
  activities: [] as ActivityPeriod<'month'>[],
});
...
const options = computed<ComposeOption<GridComponentOption | TooltipComponentOption>>(() => ({
      ...
      return `
        <dl class="flex flex-col min-w-48 items-end gap-1">
          ...
          <p class="flex flex-row gap-1 items-center text-base font-normal">
            ${i18n.t('stats.activity.monthly.graph.tooltip.newMembers.label', {
          ...
          <p class="flex flex-row gap-1 items-center text-base font-normal">
            ${i18n.t('stats.activity.monthly.graph.tooltip.members.label', {
          ...
          <p class="flex flex-row gap-1 items-center text-base font-normal">
            ${i18n.t('stats.activity.monthly.graph.tooltip.activity.label', {
  ...
  xAxis: {
    axisLabel: {
      formatter: (value: string) => dayjs(value).format('MMM').slice(0, -4),
      align: 'center',
    },
  },
}));
...
const fetchActivities = (from: string, to: string) => {
  ...
  Promise.all([
    getActivityPerMonth(from, to)
      .then((activities) => {
        state.activities = activities;
        if (!state.activities.length) {
          notificationsStore.addNotification({
            type: 'warning',
            message: i18n.t('stats.activity.monthly.onFetch.empty'),
          });
        }
      })
