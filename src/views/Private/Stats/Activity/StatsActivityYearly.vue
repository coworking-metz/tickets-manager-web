<template>
  <div class="mx-auto flex w-full max-w-5xl grow flex-col">
    <Head>
      <title>{{ $t('stats.activity.yearly.head.title') }}</title>
    </Head>
    ...
    <section class="mx-3 sm:mx-6">
      <h3 class="text-lg font-medium leading-6 text-gray-900 dark:text-gray-100">
        {{ $t('stats.activity.yearly.summary.label') }}
      </h3>
      <dl
        ...
        <div class="px-4 py-5 sm:p-6">
          <dt class="truncate font-medium text-gray-500 sm:text-sm dark:text-gray-400">
            {{ $t('stats.activity.yearly.summary.average.people.label') }}
          </dt>
                ...
                (count: number) =>
                  `${
                    count
                      ...
                      : ''
                  }${$t('stats.activity.yearly.summary.average.people.format', {
                    count,
                  })}`
              "
        ...
        <div class="px-4 py-5 sm:p-6">
          <dt class="truncate font-medium text-gray-500 sm:text-sm dark:text-gray-400">
            {{ $t('stats.activity.yearly.summary.average.attendance.label') }}
          </dt>
          <dd class="mt-1 flex items-baseline justify-between md:block lg:flex">
            <div
              v-if="state.isFetchingActivities"
              ...
              :format="
                (count: number) =>
                  `${formatAmount(count, {
                    ...
                    style: 'decimal',
                  })}${$t('stats.activity.yearly.summary.average.attendance.format', {
                    count,
                  })}`
              "
          ...
          <div v-else class="flex flex-row items-baseline justify-between text-sm">
            <span class="shrink grow basis-0 truncate font-normal text-gray-500 dark:text-gray-400">
              {{ $t('stats.activity.yearly.summary.average.attendance.description') }}
            </span>
          </div>
        </div>
      </dl>
...
import { ActivityPeriod, getActivityPerYear } from '@/services/api/activity';
...
const state = reactive({
  ...
  activities: [] as ActivityPeriod<'year'>[],
});
        ...
        <dl class="flex flex-col min-w-48 items-end gap-1">
          ...
          <dd class="flex flex-col items-end">
            <span class="text-3xl font-semibold text-gray-900 dark:text-gray-100">${
              coworkersCount ||
              i18n.t('stats.activity.yearly.graph.tooltip.total', {
          ...
          <p class="flex flex-row gap-1 items-center text-base font-normal">
            ${i18n.t('stats.activity.yearly.graph.tooltip.newMembers.label', {
          ...
          <p class="flex flex-row gap-1 items-center text-base font-normal">
            ${i18n.t('stats.activity.yearly.graph.tooltip.members.label', {
          ...
          <p class="flex flex-row gap-1 items-center text-base font-normal">
            ${i18n.t('stats.activity.yearly.graph.tooltip.activity.label', {
...
const fetchActivities = (from: string, to: string) => {
  ...
  Promise.all([
    getActivityPerYear(from, to)
      .then((activities) => {
        state.activities = activities;
        if (!state.activities.length) {
          notificationsStore.addNotification({
            type: 'warning',
            message: i18n.t('stats.activity.yearly.onFetch.empty'),
          });
        }
      })
